...
export default function SettingsBtnItem({ onClick, isCurrent = false, children, label, className }: SettingsBtnItemProps) {
    ...
    return (
        <li>
            <button onClick={onClick} className={clsx("flex cursor-pointer items-center gap-2 py-3 text-sm md:border-none md:px-3 md:rounded-lg", isCurrent && 'text-primary-blue bg-gray-neutral', className)}>
                {children}
